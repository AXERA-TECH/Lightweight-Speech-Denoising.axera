/**
 * @file test_cache_denoise.c
 * @brief cache-only 语音增强测试程序 — x86 ONNX Runtime 版本
 *
 * Usage:
 *   ./test_cache_denoise <input.wav> <output.wav> <cache_config.ini> [model.onnx]
 *
 * 示例:
 *   ./test_cache_denoise data/sample_1_noisy.wav output/out_ax620_cache.wav \
 *       models/ax620_cache_config.ini
 */
...
int main(int argc, char *argv[]) {
    if (argc < 4 || argc > 5) {
        fprintf(stderr,
            "Usage: %s <input.wav> <output.wav> <config.ini> [model.onnx]\n\n"
            "  Cache configs:\n"
            "    models/ax525_cache_config.ini\n"
            "    models/ax620_cache_config.ini\n"
            "    models/teacher_cache_config.ini\n"
            "    models/gtcrn_stream_config.ini\n",
            argv[0]);
        return 1;
    }
    ...
    printf("========================================\n");
    printf(" Cache-only Speech Enhancement (x86)\n");
    printf("========================================\n\n");
    ...
    printf("[Config] hop_len=%d  t_model=%d  f_freqs=%d  f_int=%d  step=%d  model_type=%d\n\n",
           cfg.hop_len, cfg.t_model, cfg.f_freqs, cfg.f_int, cfg.step, cfg.model_type);
    ...
    for (AX_S32 chunk = 0; chunk < n_chunks; chunk++) {
        ...
        /* 批量前处理: 一次调用，传入 step*hop_len 个连续 PCM */
        ...
        /* 批量后处理: 一次调用，输出 step*hop_len 个连续 PCM */
